#!/usr/bin/env bats
# tests/review.bats — tests for review-handler draft PR detection.
...
@test "review-handler draft: isDraft=true → removes review labels, adds draft, posts comment, exits 0" {
    ...
    # Stubs for backend functions and gh
    ...
    grep -q "Draft state"           "$comment_log"
}

@test "review-handler draft: isDraft=false → does not add draft label" {
    local ops_log="$BATS_TMPDIR/label-ops.log"
    rm -f "$ops_log"
