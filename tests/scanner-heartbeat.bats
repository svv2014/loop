...
SH
    ...
    # Silence noise and no-op network calls.
    log() { :; }
    dispatch_direct() { :; }
    scan_project() { :; }
...
# ---------------------------------------------------------------------------
# Heartbeat in scanner.sh run_once
# ---------------------------------------------------------------------------
...
@test "run_once: heartbeat mtime is updated on every tick" {
...
@test "scanner-watchdog: treats absent heartbeat as stale" {
    rm -f "$LOOP_LOG_DIR/scanner-heartbeat"
    LOOP_SCANNER_STALE_THRESHOLD=1 \
    run bash "$REPO_ROOT/scanner/scanner-watchdog.sh" --dry-run
    [ "$status" -eq 0 ]
