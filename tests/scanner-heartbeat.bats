...
#   3. scanner-watchdog.sh detects a stale heartbeat and reports "stale".
#   4. scanner-watchdog.sh --dry-run does not kill any PID.
...
@test "scanner-watchdog: verdict=stale when heartbeat is old" {
    # Backdate the heartbeat file by 1200 seconds (> default 2×300 threshold).
    ...
    run env LOOP_LOG_DIR="$LOOP_LOG_DIR" \
        ...
        "$REPO_ROOT/scanner/scanner-watchdog.sh" --dry-run
    [ "$status" -eq 0 ]
    [[ "$output" == *"verdict=stale"* || "$output" == *"stale"* ]]
}

@test "scanner-watchdog: absent heartbeat with no lock is non-fatal" {
