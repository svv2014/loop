...
@test "fallback: comment notes issue lacked criteria" {
    ...
    echo "$comment" | grep -q "were skipped"
}
...
@test "regression: finds existing bats test for scanner module" {
    local repo_root="$LOOP_ROOT"
    result=$(_find_bats_test "$repo_root" "scanner/scanner.sh")
    [ -n "$result" ]
    echo "$result" | grep -q "scanner.bats"
}
