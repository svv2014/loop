...
#   LOOP_WATCHDOG_STALE_FACTOR   multiplier × LOOP_SCANNER_INTERVAL to
#                                derive stale threshold (default 2 = 10 min)
...
if [ -n "$scanner_pid" ] && kill -0 "$scanner_pid" 2>/dev/null; then
    ...
    log "scanner lock PID '${scanner_pid}' not found or not alive — removing stale lock"
    rm -f "$LOCK_FILE"
fi
