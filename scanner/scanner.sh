...
# _write_heartbeat — touch ${HEARTBEAT_FILE} with current epoch so the
# watchdog can detect a wedged scanner by comparing mtime to now.
_write_heartbeat() {
    printf '%s\n' "$(date +%s)" > "$HEARTBEAT_FILE" 2>/dev/null || true
}

# _check_stdout_health — if the log file is no longer writable (e.g. rotated
# away and our open FD points at a deleted inode), reopen it. If reopening
# also fails, exit so launchd restarts us with clean FDs.
_check_stdout_health() {
    [ -n "${LOG_FILE:-}" ] || return 0
    if [ ! -w "$LOG_FILE" ] 2>/dev/null || ! ( echo "" >> "$LOG_FILE" 2>/dev/null ); then
