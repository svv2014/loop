...
run_once() {
    ...
    # Heartbeat — write current epoch + dedup file count so the scanner
    # watchdog can detect a wedged process (alive PID, no tick activity).
    # Written before any blocking I/O so a hang elsewhere shows up via mtime.
    if ! $DRY_RUN; then
    ...
    # writable (e.g. permissions changed by a rotation script), exit so
    # launchd restarts the process with a fresh file descriptor.
