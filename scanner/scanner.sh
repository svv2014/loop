...
# _scanner_heartbeat — stamp the heartbeat file so the external watchdog can
# confirm the scanner is making forward progress every tick.
_scanner_heartbeat() {
    printf '%s %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$$" > "${HEARTBEAT_FILE}" || true
}

# _scanner_check_stdout — if the log file has become unwritable (e.g., the log
# directory was deleted or the FD was silently closed) exit so launchd/cron
# can restart the scanner with a fresh file descriptor.
