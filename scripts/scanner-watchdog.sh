...
# 2 × LOOP_SCANNER_INTERVAL, minimum 900s / 15 min), the scanner is considered
# wedged and is killed. On macOS, launchctl kickstart restarts it immediately
# (KeepAlive=true). On Linux, the scanner's own cron entry picks it up on the
# next scheduled run.
#
...
# Threshold = 2 × poll interval, floored at 900s (15 min).
...
# Heartbeat file must exist and be writable by the time the watchdog fires.
# If it has never been written (fresh install, first boot), do nothing — the
# scanner may still be in its first tick.
...
log "heartbeat age=${age}s threshold=${THRESHOLD}s heartbeat=${HEARTBEAT_FILE}"
...
# Read the scanner's lock file to get its PID.
scanner_pid=""
if [ -f "$LOCK_FILE" ]; then
...
# Kill the wedged scanner so launchd (KeepAlive) or cron restarts it.
...
# On macOS, prod launchd to restart immediately (KeepAlive already does this
# once the process is gone, but kickstart avoids the ThrottleInterval delay).
