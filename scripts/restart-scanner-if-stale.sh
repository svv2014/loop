...
# If the heartbeat is absent or older than STALE_THRESHOLD_SECONDS (default:
# 2 × poll interval = 600s), the scanner is considered wedged and restarted:
#   macOS (launchd): launchctl kickstart the scanner service.
#   Linux (cron):    pkill the scanner process; cron re-invokes on next tick.
#
...
_restart_scanner_launchd() {
    ...
    uid=$(id -u)
    if $DRY_RUN; then
        log "DRY-RUN: would run: launchctl kickstart -k gui/${uid}/${label}"
        return
    fi
    log "restarting scanner via launchctl kickstart -k gui/${uid}/${label}"
...
_restart_scanner_linux() {
    if $DRY_RUN; then
        log "DRY-RUN: would run: pkill -f scanner.sh (cron re-invokes next tick)"
        return
    fi
    log "restarting scanner via pkill (cron re-invokes on next tick)"
