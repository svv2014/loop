...
# STALE_THRESHOLD seconds (default: 900 s / 15 min), the scanner is considered
# wedged. The lock file at /tmp/loop-scanner.lock is read for the PID; if alive
# it is sent SIGTERM (then SIGKILL if needed) so launchd (KeepAlive=true on
# com.user.loop-scanner) auto-restarts the process.
#
...
STALE_THRESHOLD="${LOOP_SCANNER_STALE_THRESHOLD:-900}"   # 15 min default
...
# Prints seconds since the file was last modified.
# Prints -1 if the file does not exist.
_file_age_seconds() {
    local path="$1"
    if [ ! -f "$path" ]; then
        echo -1
...
if [ ! -f "$LOCK_FILE" ]; then
    log "no lock file found — scanner is not running; launchd will restart it automatically"
    exit 0
fi
...
# Attempt launchctl kickstart as a belt-and-suspenders fallback for environments
# where KeepAlive may not be configured (Linux cron installs).
